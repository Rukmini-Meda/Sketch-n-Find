...
def evaluate(batch_size, dataloader_fn, images_model, sketches_model, label2index, k = 5, num_display = 2):
    ...
    with torch.no_grad():
        for iteration, batch in enumerate(images_dataloader):
            ...
            image_label_indices.append(label_indices)
    ...
    with torch.no_grad():
        for iteration, batch in enumerate(sketches_dataloader):
            ...
            sketch_label_indices.append(label_indices)
    ...
    for i in range(sketch_label_indices.shape[0]):
        average_precision_scores.append(average_precision_scores(is_correct_label_index[i], similarity[i]))
    average_precision_scores = np.array(average_precision_scores)

    index2label = {v: k for k, v in label2index.items()}
    for classes in set(sketch_label_indices):
        print('Class: %s, mAP: %f' %(index2label[classes], average_precision_scores[sketch_label_indices == cls].mean()))
